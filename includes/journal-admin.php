 ...
 * manage_ripm_journal_enqueue_admin_scripts()
 ...
 *
 */
add_filter( 'manage_posts_columns', 'manage_ripm_journal_posts_columns', 10, 2 );
function manage_ripm_journal_posts_columns( $columns, $post_type ) {
    ...
    switch ( $post_type ) {
        ...
        case 'ripm_journal':
            ...
            foreach( $columns as $key => $value ) {
                 ...
                 */
                if ( $key == 'title' ) {
                    $new_columns[ 'ripm_journal_meta_box_abbreviation' ] = 'Abbreviation';
                    $new_columns[ 'ripm_journal_meta_box_start_year' ] = 'Year';
                    $new_columns[ 'ripm_journal_meta_box_sort_title' ] = 'Sort Title';
                }
 ...
 *
 * The 'edit-ripm_journal_' section of the filter name is the custom part
 * of the filter name, which tells WordPress you want this to run
 * on the main 'ripm_journal_' custom post type edit screen. So, e.g., if
 ...
 */
add_filter( 'manage_edit-ripm_journal_sortable_columns', 'manage_ripm_journal_sortable_columns' );
function manage_ripm_journal_sortable_columns( $sortable_columns ) {
     ...
     * manage_ripm_journal_posts_columns()
     * function.
     *
     ...
     * http://mywebsite.com/wp-admin/edit.php?post_type=ripm_journal_&orderby=release_date&order=asc
     */
    $sortable_columns[ 'ripm_journal_meta_box_abbreviation' ] = 'ripm_journal_meta_box_abbreviation';

    // Let's also make the film rating column sortable
    $sortable_columns[ 'ripm_journal_meta_box_start_year' ] = 'ripm_journal_meta_box_start_year';
 ...
 */
add_action( 'manage_posts_custom_column', 'manage_ripm_journal_posts_custom_column', 10, 2 );
function manage_ripm_journal_posts_custom_column( $column_name, $post_id ) {

    switch( $column_name ) {

        case 'ripm_journal_meta_box_start_year':

            echo '<div id="ripm_journal_meta_box_start_year-' . $post_id . '">' . get_post_meta( $post_id, 'ripm_journal_meta_box_start_year', true ) . '</div>';
            break;

        case 'ripm_journal_meta_box_abbreviation':

            echo '<div id="ripm_journal_meta_box_abbreviation-' . $post_id . '">' . get_post_meta( $post_id, 'ripm_journal_meta_box_abbreviation', true ) . '</div>';
            break;

        case 'ripm_journal_meta_box_sort_title':

            echo '<div id="ripm_journal_meta_box_sort_title-' . $post_id . '">' . get_post_meta( $post_id, 'ripm_journal_meta_box_sort_title', true ) . '</div>';
            break;
 ...
 */
add_action( 'pre_get_posts', 'manage_ripm_journal_pre_get_posts', 1 );
function manage_ripm_journal_pre_get_posts( $query ) {
    ...
    if ( $query->is_main_query() && ( $orderby = $query->get( 'orderby' ) ) ) {
        ...
        switch( $orderby ) {
            ...
            case 'ripm_journal_meta_box_sort_title':
                ...
                $query->set( 'meta_key', 'ripm_journal_meta_box_sort_title' );
            ...
            case 'ripm_journal_meta_box_abbreviation':
                ...
                // set our query's meta_key, which is used for custom fields
                $query->set( 'meta_key', 'ripm_journal_meta_box_abbreviation' );
            ...
            case 'ripm_journal_meta_box_start_year':
                ...
                // set our query's meta_key, which is used for custom fields
                $query->set( 'meta_key', 'ripm_journal_meta_box_start_year' );
 ...
 */
add_filter( 'posts_clauses', 'manage_ripm_journal_posts_clauses', 1, 2 );
function manage_ripm_journal_posts_clauses( $pieces, $query ) {
    global $wpdb;
 ...
 */
//add_action( 'bulk_edit_custom_box', 'manage_ripm_journal_bulk_quick_edit_custom_box', 10, 2 );
//add_action( 'quick_edit_custom_box', 'manage_ripm_journal_bulk_quick_edit_custom_box', 10, 2 );
//function manage_ripm_journal_bulk_quick_edit_custom_box($post_id, $column_name, $post_type ) {
//
//    switch ( $post_type ) {
//
//        case 'ripm_journal':
...
<!--                        <label>-->
<!--                            <span class="title">Start Year</span>-->
<!--                            <span class="input-text-wrap">-->
<!--									<input type="text" value="" name="ripm_journal_meta_box_start_year">-->
<!--								</span>-->
<!--                        </label>-->
<!--                    </div>-->
<!--                </fieldset>--><?php
...
<!--                            <span class="input-text-wrap">-->
<!--									<input type="text" value="" name="ripm_journal_meta_box_abbreviation">-->
<!--								</span>-->
<!--                        </label>-->
<!--                    </div>-->
<!--                </fieldset>--><?php
 ...
 */
//add_action( 'admin_print_scripts-edit.php', 'manage_ripm_journal_enqueue_admin_scripts' );
//function manage_ripm_journal_enqueue_admin_scripts() {
//
 ...
 */
//add_action( 'save_post', 'manage_ripm_journal_save_post', 10, 2 );
//function manage_ripm_journal_save_post( $post_id, $post ) {
//
...
//             */
//            $custom_fields = array( 'ripm_journal_meta_box_abbreviation', 'ripm_journal_meta_box_sort_title', 'ripm_journal_meta_box_start_year' );
//
